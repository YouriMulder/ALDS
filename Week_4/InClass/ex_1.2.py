class BSTNode:
	...
	def inorder_traversal(self, parent, sortedElements = []):
		if parent.left:
			self.inorder_traversal(parent.left, sortedElements)
		sortedElements.append(parent.element)

		if parent.right:
			self.inorder_traversal(parent.right, sortedElements)

		return sortedElements

	def tree_depth(self, parent, depth = 1, maxDepth = 1):
		if not parent.left and not parent.right:
			return maxDepth
		
		if parent.left:
			depthLeft = self.tree_depth(parent.left, depth + 1)
		if parent.right:
			depthRight = self.tree_depth(parent.right, depth + 1)
		
		if depthLeft and maxDepth < depthLeft:
			maxDepth = depthLeft
		if depthRight and maxDepth < depthRight:
			maxDepth = depthRight
...
class BST:
	...
	def isBalanced(self):
		print(self.root.tree_depth(b.root))
		return self.root.inorder_traversal(b.root)
